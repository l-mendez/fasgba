...
// GET /api/tournaments/[id]/teams - Get all available clubs for team tournament match setup
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Get all clubs (teams) available for team tournaments
    const { data: clubs, error } = await serverSupabase
      .from('clubs')
      .select('id, name')
